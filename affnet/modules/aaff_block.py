...
class AFNO2D_channelfirst(nn.Module):
    ...
    def forward(self, x, spatial_size=None):
        ...
        x = torch.fft.rfft3(x, dim=(2, 3, 4), norm="ortho")
        origin_ffted = x
        ...
        x = x.reshape(B, C, D, x.shape[3], x.shape[4])

        x = x * origin_ffted
        x = torch.fft.irfft3(x, s=(H, W, D), dim=(2, 3, 4), norm="ortho")
        x = x.type(dtype)
...
def save_kernel(origin_ffted, H, W, D):
    import time
    import matplotlib.pyplot as plt
    import os
    now = time.time()
    origin_ffted = origin_ffted.detach()
    kernel = torch.fft.irfft3(origin_ffted, s=(H, W, D), dim=(2, 3,4), norm="ortho")
