...
class AFNO1D_channelfirst(nn.Module):
    """
    ...
    def forward(self, x, spatial_size=None):
        ...
        origin_ffted = x
        x = x.reshape(B, self.num_blocks, self.block_size, x.shape[2])
...
class AFNO3D_channelfirst(nn.Module):
    """
    hidden_size: channel dimension size
    num_blocks: how many blocks to use in the block diagonal weight matrices (higher => less complexity but less parameters)
    sparsity_threshold: lambda for softshrink
    ...
    def forward(self, x, spatial_size=None):
        ...
        x = torch.fft.rfft2(x, dim=(2, 3, 4), norm="ortho")
        origin_ffted = x
        x = x.reshape(B, self.num_blocks, self.block_size, x.shape[2], x.shape[3], x.shape[4])


        o1_real = self.act(
            torch.einsum('bkihwn,kio->bkohwn', x.real, self.w1[0]) - \
            torch.einsum('bkihwn,kio->bkohwn', x.imag, self.w1[1]) + \
            self.b1[0, :, :, None, None, None]
        )

        o1_imag = self.act2(
            torch.einsum('bkihwn,kio->bkohwn', x.imag, self.w1[0]) + \
            torch.einsum('bkihwn,kio->bkohwn', x.real, self.w1[1]) + \
            self.b1[1, :, :, None, None,None]
        )

        o2_real = (
                torch.einsum('bkihwn,kio->bkohwn', o1_real, self.w2[0]) - \
                torch.einsum('bkihwn,kio->bkohwn', o1_imag, self.w2[1]) + \
                self.b2[0, :, :, None, None,None]
        )

        o2_imag = (
                torch.einsum('bkihwn,kio->bkohwn', o1_imag, self.w2[0]) + \
                torch.einsum('bkihwn,kio->bkohwn', o1_real, self.w2[1]) + \
                self.b2[1, :, :, None, None,None]
        )

        x = torch.stack([o2_real, o2_imag], dim=-1)
        x = F.softshrink(x, lambd=self.sparsity_threshold)
        x = torch.view_as_complex(x)
        x = x.reshape(B, C, x.shape[3], x.shape[4], x.shape[5])

        x = x * origin_ffted
        x = torch.fft.irfft2(x, s=(H, W, N), dim=(2, 3, 4), norm="ortho")
        x = x.type(dtype)
...
class AFFBlock(BaseModule):
    ...
    def forward_spatial_3d(self, x: Tensor) -> Tensor:
        res = x
        ...
        b, c, h, w, n = x.size()
        patches = einops.rearrange(fm, 'b c h w n -> b (h w n) c')

        # learn global representations
        for transformer_layer in self.global_rep:
            patches = transformer_layer(patches)

        fm = einops.rearrange(patches, 'b (h w n) c -> b c h w n', h=h, w=w, n=n)
